...
async def detect(files: List[UploadFile] = File(...), _ = Depends(verify_api_key)):
    ...
    for file in files:
        ...
        for name, crop_img in crops:
            ...
            # Draw a smaller header
            cv2.rectangle(annotated_frame, (0, 0), (200, 30), (0, 0, 0), -1)
            cv2.putText(annotated_frame, name, (10, 22), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2, cv2.LINE_AA)

            # Fix blue tint: convert BGR to RGB (OpenCV plot is BGR)
            # Actually, if it was ALREADY blue, removing the swap or fixing the input might be needed.
            # Usually results.plot() returns BGR. If the output was blue with [..., ::-1], 
            # it means annotated_frame was ALREADY RGB.
